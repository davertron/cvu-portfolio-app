        ...
        try {
            ...
            await Promise.all(
                Object.keys(collectionMap).map(async (cid) => {
                    const [driveCollection, _artifacts] = await drive.file_collections.load([collectionMap[cid], []]);
        ...
        >
            <div className="flex flex-col items-center bg-gradient-to-r from-blue-300 to-indigo-700 w-full py-10 text-white">
                <h1 className="font-bold text-3xl text-center my-4">
                    {user?.name && !owned ? user.name + "'s Blog" : 'Blog'}
                </h1>
                <div
                ...
                >
                    ...
                    >
                        <div>
                            <MdSearch
                                size="1.3em"
                                className={classNames('stroke-1', searchFocused ? 'text-white' : 'text-indigo-200')}
                            />
                        </div>
                    </div>
                    <Input
                    ...
                    >
                        <Button
                            ...
                            className={classNames(search.term?.trim?.() == '' && 'opacity-0')}
                        >
                            <MdClose size="1.3em" className="text-indigo-200 hover:text-white" />
                        </Button>
                    </div>
                </div>
            </div>
            <div className="flex items-start px-20 py-7 relative">
                <div className="flex-grow flex flex-col items-center px-10 sm:px-5">
                    {error && (
                        <div className="self-stretch mb-1">
                            <Error error={error} />
                        </div>
                    )}
                    {search.term?.trim?.() == '' &&
                        filters.length == 0 &&
                        posts.filter((post) => post.awaiting_save).length == 0 &&
                        owned && (
                            <Cta
                    ...
                    {search.term?.trim?.() == '' ? (
                        [
                            ...
                            ...posts
                                .filter((p) => !p.awaiting_save)
                                .sort(createdAt)
                                .filter(matchFilters(filters))
                                .map(renderPost),
                        ]
                    ) : searchResults.length > 0 ? (
                        searchResults.filter(matchFilters(filters)).map(renderPost)
                    ) : (
                                    ...
                                    onClick={(selected) => {
                                        if (selected) {
                                            ...
                                            setFilters((currentFilters) =>
                                                currentFilters.filter((filter) => filter != cid)
                                            );
                                        }
                                    }}
...
function BlogPost(props: PostProps) {
    ...
    async function save() {
        try {
            ...
            await db.posts.doc(post.id).set(post);
        } catch (_e) {
            props.setError('There was an error saving your post');
        }
    }
    ...
    async function cancel() {
        try {
            ...
            props.setError('There was an error resetting your post');
        }
    }
    ...
    async function getData(pid: string) {
        try {
            ...
            props.setError('There was an error retrieving comments');
        }
    }
    ...
    async function saveComment(comment: Comment) {
        try {
            ...
            if (comment.awaiting_save) {
                setComment(comment.id)((currentComment) =>
                    currentComment.with({
                        awaiting_save: false,
                    })
                );
    ...
    return (
        ...
        >
            <div className="my-3">
                {editing ? (
                    <Input
                        type="text"
                        name="title"
                        className="text-xl font-bold w-full text-gray-600"
                        placeholder="Post title"
                        setForm={props.setPost}
                        value={post?.title}
                    />
                ) : (
            ...
            {(post.tags.length > 0 || editing) && (
                <>
                    <h3 className="font-bold my-1 text-gray-700">
                        {editing ? 'Tag collections' : 'Tagged collections'}
                    </h3>
            ...
            {editing && (
                <div className="mb-4 mt-6 flex">
                    <Cta className="mr-4" disabled={!props.validate(post)} icon={<MdDone />} onClick={save}>
                        Save
                    </Cta>
                    <OutlineButton color="indigo-500" icon={<MdClose />} onClick={cancel}>
                        Cancel
                    </OutlineButton>
                </div>
            )}
            {!(editing || props.locked) && (
                <div className="flex flex-col mt-2">
                    <Button onClick={() => setExpand(!expand)} className="text-right text-lg mb-2" customPadding>
                        {expand ? <MdExpandLess /> : <MdMoreHoriz />}
                    </Button>
                    {expand && (
                        <div className="flex mt-1 mb-3">
                            ...
                            <OutlineButton color="red-700" onClick={deletePost} icon={<MdClose />}>
                                Delete
                            </OutlineButton>
                        </div>
                    )}
            ...
            {(comments.length > 0 || props.locked) && (
                <div className="mt-3 mb-1">
                    {comments.sort(createdAt).map((comment) => {
                        const showInputs =
                            (comment.awaiting_save || editingId == comment.id) &&
                                    ...
                                    {showInputs ? (
                                        ...
                                        <>
                                            <div className="flex flex-wrap justify-between flex-grow mb-1 w-full">
                                                ...
                                                {comment.created_at && (
                                                    <p className="text-gray-400">{dateString(comment.created_at)}</p>
                                                )}
                                            </div>
                    ...
                    {(post.likes.length > 0 || props.locked) && (
                        <div className="flex justify-between mt-2 items-center">
                            ...
                            <div className="flex flex-grow justify-end items-center">
                                <Button
                                    ...
                                    onClick={like}
                                    icon={
                                        session && post?.likes?.indexOf?.(session?.user?.id) == -1 ? (
                                            <MdFavoriteBorder />
                                        ) : (
                                            <MdFavorite />
                                        )
