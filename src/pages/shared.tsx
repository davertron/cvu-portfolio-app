...
export default function Shared() {
    ...
    async function getDriveData(client) {
        try {
            ...
            const driveCollections = await Promise.all(
        ...
        <Layout authorization={Authorization.USER} gapis={[]} onGapisLoad={() => setApisLoaded(true)} noPadding>
            <div className="flex flex-col items-center bg-gradient-to-r from-indigo-500 to-purple-500 w-full py-3 text-white">
                ...
                <div className="mx-auto my-3 p-1 rounded bg-gray-100 bg-opacity-10 flex">
                    <SwitchButton active={showProfiles} icon={<MdPeople />} onClick={() => setShowProfiles(true)}>
                        Profiles
                    </SwitchButton>
                    <SwitchButton active={!showProfiles} icon={<MdPermMedia />} onClick={() => setShowProfiles(false)}>
                        Collections
                    </SwitchButton>
                </div>
            </div>
                ...
                {showProfiles ? (
                    Object.keys(users).length > 0 ? (
                        Object.keys(users).map((uid) => (
                            <div key={uid}>
                                <div
                                    className="m-4 bg-gray-100 shadow rounded bg-cover backdrop-blur"
                                    style={{ backgroundImage: `url(${users[uid]?.bio_pic?.url})` }}
                                >
                                    <div className="bg-indigo-700 bg-opacity-80 rounded-md text-white py-3 px-5">
                                        <div className="pt-1 pb-3 font-bold text-lg flex items-center">
                                            <img
                                                src={users[uid]?.image}
                                                className="flex-shrink-0 h-7 w-7 mr-3 rounded-full"
                                            />
                                            <p>{users[uid]?.name}</p>
                                        </div>
                                        <p className="py-1">{users[uid]?.role?.toTitleCase?.()}</p>
                                        <div className="py-2 flex text-sm justify-between">
                                            <Cta icon={<MdPersonOutline />} href={'/users/' + uid} customFont invert>
                                                Profile
                                            </Cta>
                                            <OutlineButton
                                                color="indigo-400"
                            ...
                            <div className="m-4 bg-indigo-100 shadow rounded w-56">
                                <div className="text-gray-600 px-4 pt-1 pb-2">
                                    <div className="flex items-start">
                                        <p className="text-lg font-bold my-2 h-full flex-grow">{collection.title}</p>
                                    </div>
