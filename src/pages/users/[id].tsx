...
export default function Profile() {
    ...
    const visibleSharedWith = user?.shared_with
        ? user.shared_with.filter((permission) => !permission.awaiting_delete)
        : [];
    ...
    const validate = (email: string) => {
        ...
        if (parts.length == 2) {
            ...
            return (
                ...
                domain == 'cvsdvt.org' &&
                sharingError == null &&
                user?.shared_with?.filter((p) => !p.awaiting_delete && p.email == email).length == 0
            );
        }
    ...
    async function share() {
        try {
            ...
            if (snapshot.empty) {
                ...
                setUser((currentUser) =>
                    currentUser.with({
                        shared_with: [...currentUser.shared_with, new Permission({ email: user.email })],
                    })
                );
    ...
    async function save() {
        try {
            ...
            let driveSharedWith = await Promise.all(
                user.shared_with.map(async (permission) => {
            ...
            >
                ...
                {user && session && (
                    <div>
                        {session.user?.id == id && (
                            <div
                                className="rounded-lg p-2 border border-gray-300 flex justify-center top-0 bg-white z-10 transition-all"
                                id="sticky-buttons"
                            >
                                {editing && (
                                    <Cta
                        ...
                        <div className="flex flex-wrap-reverse justify-center w-screen rounded py-3 px-5 text-gray-600 relative">
                            <div className="m-8 flex-column w-80">
                                {editing ? (
                                    <>
                                        <Input type="file" setFiles={setFiles}>
                                            <div className="relative">
                                                <img
                                                    src={files?.length > 0 ? files[0].preview?.url : user.bio_pic?.url}
                                                    alt="Bio pic"
                                ...
                                {files?.length > 0 && editing && (
                                    <div>
                                        <hr />
                                        <div>
                                            <p className="text-gray-400 my-4" onClick={(_e) => setFiles([])}>
                                            ...
                                            {visibleSharedWith?.length > 0 && (
                                                <>
                                                    <p className="text-gray-500 pt-3">
                                                        Users who can see your portfolio
                                                    </p>
                                                    <div className="py-1">
                                                        {visibleSharedWith.map(
                                                            (permission) =>
                                                                sharedWith[permission.email] && (
                                                                    <div
                                                                        className="my-3 bg-gray-50 rounded px-2 py-2 flex"
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <div className="mr-3 flex-grow">
                                                                            <h4 className="font-bold">
                                                                                {sharedWith[permission.email].name}
                                                                            </h4>
                                                                        ...
                                                                        {editing && (
                                                                            <div>
                                                                                <Button
                                                                                    onClick={() => {
                                                                                        setPermission(permission.email)(
                                                                                            (currentPermission) => ({
                                                                                                ...currentPermission,
