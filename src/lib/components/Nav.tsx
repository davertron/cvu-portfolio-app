        ...
        return (
            <Menu>
                <Menu.Button className="focus:outline-none">
                    <img
                        src={props.img}
                        className={classNames('h-8 w-8 rounded-full align-middle cursor-pointer')}
                        alt={props.alt}
                    />
                </Menu.Button>
                <Menu.Items className="origin-top-right absolute right-0 mt-11 w-48 rounded border border-gray-200 bg-white shadow-lg focus:outline-none z-20">
                    {props.dropdown.map((option) => (
                        <Menu.Item key={option.children as string}>{(_e) => <DropdownLink {...option} />}</Menu.Item>
                    ))}
                </Menu.Items>
            </Menu>
        );
...
function DropdownLink(props: LinkProps) {
    return <NavLink {...props} display="block" />;
}
