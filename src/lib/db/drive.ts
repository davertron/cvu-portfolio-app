...
export default class DriveDb {
    ...
    file_collections: DriveHandler<[FileCollection, Artifact[]]> = {
        save: DriveDb.exec_safe(
            async ([collection, artifacts]: [FileCollection, Artifact[]]): Promise<[FileCollection, Artifact[]]> => {
                ...
                artifacts = await Promise.all(
                    artifacts.map(async (artifact) => await this.artifacts.save(artifact, collection.drive_id))
                );
        ...
        load: DriveDb.exec_safe(
            async ([collection, artifacts]: [FileCollection, Artifact[]]): Promise<[FileCollection, Artifact[]]> => {
                ...
                artifacts = await Promise.all(artifacts.map(async (artifact) => await this.artifacts.load(artifact)));
        ...
        share: DriveDb.exec_safe(
            async ([collection, artifacts]: [FileCollection, Artifact[]], permission: Permission) => {
